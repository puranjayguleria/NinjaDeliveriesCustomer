...
const NewOrderStack = () => (
  <Stack.Navigator initialRouteName="PickupLocation">
    ...
    <Stack.Screen name="DropoffLocation" component={DropoffLocationScreen} options={{ headerShown: false }}/>
    <Stack.Screen
      name="AdditionalInfo"
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleNotificationResponse = (response) => {
      const { orderId, pickupCoords, dropoffCoords, status } =
        response.notification.request.content.data;

      if (orderId) {
        if (status === "pending") {
          navigationRef.current?.navigate("OrderAllocatingScreen", {
            orderId,
            pickupCoords,
            dropoffCoords,
          });
        } else {
          navigationRef.current?.navigate("OrderTrackingScreen", {
            orderId,
            pickupCoords,
            dropoffCoords,
          });
