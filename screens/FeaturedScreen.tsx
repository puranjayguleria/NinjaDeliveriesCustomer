...
const GRID_CARD_WIDTH = (width - CARD_MARGIN * (GRID_COLUMNS * 2)) / GRID_COLUMNS;
...
const FeaturedScreen: React.FC = () => {
    ...
    try {
      ...
      if (!doc.empty) {
        ...
        setEventAttrs({
          name: "Coming Soon",
          themeColor: "#d32f2f",
          bannerImage: "https://via.placeholder.com/700x300.png?text=Event+Banner",
          emojis: ["🎉", "✨", "🔥"],
        });
      }
    } catch (err) {
  ...
  const fetchActiveSubcats = async (storeId: string) => {
    try {
      const snap = await firestore().collection("subcategories")
  ...
  /*  DERIVED DATA                                         */
  /* ----------------------------------------------------- */
  const featuredProduct = useMemo(() => (filtered.length > 0 ? filtered[0] : null), [filtered]);
  const gridProducts = useMemo(() => (filtered.length > 1 ? filtered.slice(1) : []), [filtered]);
  ...
  const FloatingEmoji: React.FC<{ emoji: string }> = ({ emoji }) => {
    ...
    return (
      <Animated.View
        style={{ position: "absolute", bottom: 0, left: x, transform: [{ translateY }], opacity }}
      >
  ...
  const renderFeatured = () => {
    ...
    return (
      <View style={styles.featuredContainer}>
        <ImageBackground
          source={{ uri: featuredProduct.image || "https://via.placeholder.com/500" }}
        ...
        >
          <View style={styles.featuredOverlay}>
            ...
            {featuredProduct.discount && (
              <Text style={styles.featuredDiscount}>₹{featuredProduct.discount} OFF</Text>
            )}
          </View>
        </ImageBackground>
        ...
        {qty === 0 ? (
          ...
          <View style={styles.plusMinusContainer}>
            <TouchableOpacity onPress={() => decreaseQuantity(featuredProduct.id)}>
              <MaterialIcons name="remove-circle" size={32} color={eventAttrs?.themeColor || "#d32f2f"} />
            </TouchableOpacity>
            <Text style={styles.quantityText}>{qty}</Text>
            <TouchableOpacity onPress={() => increaseQuantity(featuredProduct.id, featuredProduct.quantity)}>
              <MaterialIcons name="add-circle" size={32} color={eventAttrs?.themeColor || "#d32f2f"} />
            </TouchableOpacity>
          </View>
        )}
      </View>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={[styles.container, { backgroundColor: eventAttrs?.themeColor || "#fafafa" }]}>
        {/* ---------- Banner ---------- */}
        {eventAttrs?.bannerImage ? (
          <ImageBackground source={{ uri: eventAttrs.bannerImage }} style={styles.banner} resizeMode="cover">
            <View style={styles.bannerOverlay}>
              <Text style={styles.bannerText}>{eventAttrs.name}</Text>
            </View>
          </ImageBackground>
        ) : (
          <View style={[styles.header, { backgroundColor: eventAttrs?.themeColor || "#d32f2f" }]}>
            <Text style={styles.headerTitle}>{eventAttrs?.name || "Event Specials"}</Text>
            <TouchableOpacity style={styles.continueButton} onPress={() => navigation.replace("Categories" as never)}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          {/* products grid */}
          {loading ? (
            <ActivityIndicator style={{ marginTop: 40 }} size="large" color="#28a745" />
          ) : (
            <View style={styles.gridContainer}>
              {(search.trim() !== "" ? filtered : gridProducts).map((item) => {
                ...
                return (
                  <View
                    key={item.id}
                    style={[styles.gridItemWrapper, { width: GRID_CARD_WIDTH, margin: CARD_MARGIN }]}
                  >
                    <ProductCard
...
const styles = StyleSheet.create({
  ...
  bannerOverlay: { backgroundColor: "rgba(0,0,0,0.4)", padding: 10, alignItems: "center" },
  bannerText: { fontSize: 28, fontWeight: "700", color: "#fff" },
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 12 },
  headerTitle: { fontSize: 24, fontWeight: "700", color: "#fff" },
  continueButton: { backgroundColor: "#fff", paddingVertical: 6, paddingHorizontal: 12, borderRadius: 20 },
  continueButtonText: { color: "#d32f2f", fontSize: 14, fontWeight: "600" },

  /* search */
  searchContainer: { flexDirection: "row", alignItems: "center", margin: 10, position: "relative" },
  searchIcon: { position: "absolute", left: 20, zIndex: 2 },
  searchBar: { flex: 1, backgroundColor: "#fff", borderRadius: 25, paddingLeft: 50, paddingRight: 50, height: 45, fontSize: 16, color: "#333", elevation: 3 },
  ...
  featuredPrice: { fontSize: 22, fontWeight: "600", color: "#fff", marginTop: 4 },
  featuredDiscount: { fontSize: 18, fontWeight: "600", color: "#ffeb3b", marginTop: 4 },
  addToCartButton: { backgroundColor: "#fff", paddingVertical: 8, paddingHorizontal: 16, alignSelf: "center", borderRadius: 25, marginTop: 10 },
  addToCartText: { color: "#d32f2f", fontSize: 16, fontWeight: "600" },
  plusMinusContainer: { flexDirection: "row", alignItems: "center", justifyContent: "center", marginTop: 10 },
