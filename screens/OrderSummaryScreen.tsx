...
import { calculateDistance, isWithin10KmOfDharamshala } from '../utils/locationUtils';
import { fetchFixedPickupLocation, PickupLocation } from '@/utils/fetchPickUpLocation';
...
const OrderSummaryScreen: React.FC = () => {
  ...
  const [fixedPickupLocation, setFixedPickupLocation] = useState<PickupLocation | null>(null);
  ...
  useEffect(() => {
    const fetchDistance = async () => {
      ...
      try {
        const calculatedDistance = await calculateDistance(pickupCoords, dropoffCoords);
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Order Confirmed',
        `Your order has been placed successfully. Total cost: ₹${finalTotalCost.toFixed(2)}`
      );
    } catch (error) {
        ...
        <View style={styles.detailsContainer}>
          ...
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>
              Additional Cost (₹{additionalCostPerKm}/km after {distanceThreshold} km):
            </Text>
